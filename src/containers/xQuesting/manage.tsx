...
export const QuestedGalleryItemsHeader = ({ quest, kpis }) => {
  ...
  useEffect(() => {
    let old = [];
    const kpisNormalized = kpis.filter(
            ...
            >
              <Stack>
                <Typography
                  ...
                  sx={{ paddingTop: "2px" }}
                >
                  {quests[quest].Tender.Amount /
                    Math.pow(10, quests[quest].Tender.Decimals)}{" "}
                  {quests[quest].Tender.Name}
                </Typography>
                <Typography
                  gutterBottom
                  variant="h5"
            ...
            >
              <Stack>
                <Typography
                  ...
                  sx={{ paddingTop: "2px" }}
                >
                  {questsKPIs.hasOwnProperty(quest)
                    ? Number(questsKPIs[quest].stakingRewards).toFixed(
                        quests[quest].StakingConfig.Decimals
                      )
                    : "0.0"}
                </Typography>
                <Typography
            ...
            >
              <Stack>
                ...
                >
                  {questsKPIs.hasOwnProperty(quest)
                    ? Number(
                        questsKPIs[quest].totalStaked / (20 * 1000)
                      ).toFixed(4)
                    : "0.0000"}
                  %
                </Typography>
            ...
            >
              <Stack>
                <Typography
                  ...
                  sx={{ paddingTop: "2px" }}
                >
                  {quests[quest].StakingConfig.YieldPer /
                    Math.pow(10, quests[quest].StakingConfig.Decimals)}{" "}
                  {quests[quest].StakingConfig.Name}
                </Typography>
                <Typography
                  gutterBottom
                ...
                >
                  Every{" "}
                  {getHumanReadablePluralTime(
                    quests[quest].StakingConfig.YieldPerTime
                  )}
                </Typography>
              </Stack>
            </Grid>
...
export const QuestedGalleryItems = ({ onSelection }) => {
  ...
  useEffect(() => {
    async function normalizeQuested() {
      ...
      switch (globalEnum) {
        case "recover": {
          purgatory = questsProposals.hasOwnProperty(questSelection)
      ...
      const pairs = await Promise.all(
        purgatory.map(async (item) => {
          let disabled = true;
          let remainingTime = 0;

          let elapsedSinceEnd =
            Math.round(new Date().getTime() / 1000) - item.EndTime;
          ...
          return {
            ...
            remainingTime,
            depositsMints: [
              ...(item.DepositingLeft !== null ? item.DepositingLeft : []),
              ...(item.DepositingRight !== null ? item.DepositingRight : []),
            ],
                  ...
                  .findAllByMintList(
                    ...
                    [
                      ...(item.DepositingLeft !== null
                        ? item.DepositingLeft.filter((_, index) =>
                            item.RecordLeft !== null
                              ? item.RecordLeft[index]
                              : false
                          ).map((mint) => new PublicKey(mint))
                        : []),
                      ...(item.DepositingRight !== null
                        ? item.DepositingRight.filter((_, index) =>
                            item.RecordRight !== null
                              ? item.RecordRight[index]
                              : false
                          ).map((mint) => new PublicKey(mint))
                        : []),
                    ]
  ...
  const buttonText = useCallback(
    (state) => {
      switch (globalEnum) {
        ...
        default:
          return "Congrats on the easter egg!";
      }
    },
    [globalEnum]
  );
      ...
      <Grid justifyContent="center" alignItems="center" container sx={{}}>
        {globalEnum !== "recover" && (
          <QuestedGalleryItemsHeader
            ...
            kpis={[
              quests.hasOwnProperty(questSelection) &&
              quests[questSelection].StakingConfig !== null
                ? "stakingReward"
                : "",
            ]}
          />
        ...
        {nftsQuested.map(
          ({ depositsMetadata, disabled, remainingTime }, pairIndex) => (
            <StyledCard>
                  ...
                  {depositsMetadata.map((depositMetadata, nftIndex) => (
                    ...
                    >
                      <CardMedia
                        ...
                        image={
                          //@ts-ignore
                          depositMetadata.offchainMetadata.hasOwnProperty(
                            "image"
                          )
                            ? //@ts-ignore
