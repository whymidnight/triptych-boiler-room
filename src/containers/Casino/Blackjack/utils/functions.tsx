...
function getBasicValue(faceValue: string): number {
  ...
  if (isNotNumber && faceValue.toString().toLowerCase() === "ace") {
    ...
    courtValues
      .concat(courtValues.map((value) => value.toLocaleLowerCase()))
      .includes(faceValue)
  ) {
    return 10;
  } else if (!isNotNumber) {
    if (valueToNumber < 2 || valueToNumber > 10)
      throw "Card face value is not valid: it must be: 2 < value < 10.";
...
function cardsSum(cards: cardInterface[]): number {
  ...
  while (result > 21 && values.some((value) => value == 11)) {
    values.splice(
      values.findIndex((value) => value == 11),
      1,
      1
    );
