...
export const XSwap = () => {
  ...
  const onInvokeSwap = useCallback(
    (event) => {
        ...
        const selectQuestIx = JSON.parse(
          String.fromCharCode(
            ...
            ...(await invoke_swap(
              ...
              ORACLE.toString(),
              String(swaps[selectedFrom].swapMeta.Index),
              amount.toFixed(8)
            ))
          )
        );
    ...
    (event, direction, op) => {
      ...
      switch (direction) {
        case "from": {
          switch (op) {
            case "decr": {
              const toAmountInp = Math.floor(
                Number(amount - swaps[selectedFrom].swapMeta.Per) /
                  swaps[selectedFrom].swapMeta.Per
              );
            ...
            case "incr": {
              const toAmountInp = Math.floor(
                Number(swaps[selectedFrom].swapMeta.Per + amount) /
                  swaps[selectedFrom].swapMeta.Per
              );
              const amountInp = toAmountInp * swaps[selectedFrom].swapMeta.Per;
              if (amountInp < 0 || amountInp > balance) return;
  ...
  const onAmountChange = useCallback(
    (event, fieldEnum) => {
      const toAmountInp = Math.floor(
        Number(event.target.value) / swaps[selectedFrom].swapMeta.Per
      );
      const amountInp = toAmountInp * swaps[selectedFrom].swapMeta.Per;
  ...
  useEffect(() => {
    async function fetchBalance() {
      ...
      const userTokens = await connection.getParsedTokenAccountsByOwner(
        wallet.publicKey,
        { mint: new PublicKey(swaps[selectedFrom].swapMeta.FromMint) }
      );
      if (userTokens.value.length !== 0) {
        setBalance(
          userTokens.value[0].account.data.parsed.info.tokenAmount.uiAmount
        );
        return;
      }
      setBalance(0);
  ...
  useEffect(() => {
    async function getTokens() {
      ...
      const swaps = JSON.parse(
        String.fromCharCode(...(await get_swaps(ORACLE.toString())))
      );
      console.log(swaps);
      setSwaps(
        swaps.map((swap, index) => ({
          ...swap,
          name: String("debug" + " #" + index),
        }))
      );
    }
    getTokens();
      ...
      <Box>
        <StyledCard className="swap-container">
          <StyledCard className="swap-card">
            ...
            <Typography variant="h5" component="div">
              {selectedFrom !== -1
                ? swaps[selectedFrom].mintsMeta.from.tokenMetadata.symbol
                : "Token"}
              -
              {selectedTo !== -1
                ? swaps[selectedFrom].mintsMeta.to.tokenMetadata.symbol
                : "Token"}
            </Typography>
          </StyledCard>
              ...
              >
                <Grid item xs={4}>
                  <FormControl fullWidth sx={{ textAlignLast: "center" }}>
                    <InputLabel>
                      {selectedFrom !== -1
                        ? swaps[selectedFrom].mintsMeta.from.tokenMetadata
                            .symbol
                        : "Token"}
                    </InputLabel>
                    <Select
                    ...
                    >
                      <MenuItem
                        sx={{
                          background:
                            "linear-gradient(140.14deg,rgb(0 182 191 / 15%),rgb(27 22 89 / 10%) 86.61%),linear-gradient(321.82deg,rgb(24 19 77),rgb(27 22 89))",
                        }}
                        value={-1}
                      ...
                      {swaps.map((swap, index) => (
                        <MenuItem
                          ...
                          value={index}
                          onClick={(event) =>
                            onSwapSelect(event, "from", index)
                          }
                        >
                          {swap.mintsMeta.from.tokenMetadata.name}
                        </MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                ...
                <Grid item xs={8}>
                  ...
                  >
                    <Button
                      className="swap-card"
                      onClick={(event) => onAmountUpdate(event, "from", "decr")}
                    >
                      -
                    </Button>
                    <TextField
                      ...
                      InputProps={{
                        inputProps: {
                          min: 0,
                          max: balance,
                          step: swaps.hasOwnProperty(selectedFrom)
                            ? String(swaps[selectedFrom].swapMeta.Per)
                            : "1",
                    ...
                    <Button
                      className="swap-card"
                      onClick={(event) => onAmountUpdate(event, "from", "incr")}
                    >
                      +
                    </Button>
                  </Box>
              ...
              >
                <Grid item xs={4}>
                  <FormControl fullWidth sx={{ textAlignLast: "center" }}>
                    <InputLabel>
                      ...
                      >
                        {selectedFrom !== -1
                          ? swaps[selectedFrom].mintsMeta.to.tokenMetadata
                              .symbol
                          : "Token"}
                      </Typography>
                    </InputLabel>
                        ...
                        swaps
                          .reduce((acc, item) => {
                            if (
                              !acc.some(
                                (someItem) =>
                                  someItem.swapMeta.ToMint ===
                                  item.swapMeta.ToMint
                              )
                            ) {
                              return [...acc, item];
                            }
                          ...
                          .map((swap, index) => (
                            <MenuItem
                              ...
                              value={index}
                              onClick={(event) =>
                                onSwapSelect(event, "to", index)
                              }
                            >
                              {swap.mintsMeta.to.tokenMetadata.name}
                            </MenuItem>
                          ))}
                      {swaps
                        .filter((swap, index) =>
                          selectedFrom !== -1
                            ? swaps[index].swapMeta.FromMint ===
                              swaps[selectedFrom].swapMeta.FromMint
                            : false
                        )
                        .map((swap, index) => (
                          <MenuItem
                ...
                <Grid item xs={8}>
                  ...
                  >
                    <Button
                      className="swap-card"
                      onClick={(event) => onAmountUpdate(event, "to", "decr")}
                    >
                      -
                    </Button>
                    <TextField
                    ...
                    <Button
                      className="swap-card"
                      onClick={(event) => onAmountUpdate(event, "to", "incr")}
                    >
                      +
                    </Button>
                  </Box>
